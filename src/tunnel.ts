        ...
        // Handle TCP packets (nextHeader === 6)
          ...
          if (packet.length < tcpHeaderStart + 20) {
            ...
            if (packet.length < tcpHeaderStart + tcpHeaderLength) {
              ...
              log.debug(
                `TCP packet detected: headerLength=${tcpHeaderLength}, payload length=${tcpPayload.length}`,
              );
              const packetData: PacketData = {
                protocol: 'TCP',
                src,
      ...
      try {
        ...
        if (data && data.length > 0) {
          if (data.length >= 40) {
            ...
            log.debug(
              `TUN → Device: ${data.length} bytes, IPv6 src=${formatIPv6Address(data.slice(8, 24))}, dst=${formatIPv6Address(data.slice(24, 40))}`,
            );
          } else {
...
export async function exchangeCoreTunnelParameters(socket: Socket): Promise<TunnelInfo> {
  return new Promise((resolve, reject) => {
    ...
    log.debug(
      `Sending CDTunnel packet: magic=${magic.toString()}, length=${jsonBuffer.length}, body=${requestJSON}`,
    );
